...
if [ -f "$SCRIPT_DIR/deploy.sh" ]; then
    # Extract PROJECT_PREFIX variable from deploy.sh
    DETECTED_PREFIX=$(grep -E '^PROJECT_PREFIX=' "$SCRIPT_DIR/deploy.sh" 2>/dev/null | head -1 | sed 's/PROJECT_PREFIX=["'"'"']*\([^"'"'"']*\)["'"'"']*/\1/')
    if [ -n "$DETECTED_PREFIX" ]; then
        DEFAULT_PROJECT_PREFIX="$DETECTED_PREFIX"
    fi
    
    # Extract CONNECTION_NAME from deploy.sh
...
# Use the main project schema as default (matches PROJECT_PREFIX), fall back to first if not found
# The main schema (e.g., YO_SWEET_SPOT) is where the data mart and Streamlit app objects reside
...
${SCHEMA_GRANTS}
${COMPUTE_POOL_GRANTS}
-- Note: Notebook access is implicit via role ownership (notebook is created by this role)
